...
async function pickComboboxOption(page: Page, input: Locator, searchTerm: string, expectedEmail: string) {
...
test.describe("Teams page", () => {
  ...
  test.describe("Create Team", () => {
    ...
    test("creates team with multiple members (mixed roles)", async ({ page }) => {
      ...
      expect(memberRequests.filter((m: any) => m.role === "member")).toHaveLength(2);
      expect(memberRequests.filter((m: any) => m.role === "owner")).toHaveLength(1);
    });
